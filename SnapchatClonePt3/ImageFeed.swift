...
func getPosts(user: CurrentUser, completion: @escaping ([Post]?) -> Void) {
    ...
    dbRef.child(firPostsNode).observeSingleEvent(of: .value, with: { (snapshot) in
        if ( snapshot.exists() && snapshot.value != nil ) {
            ...
            user.getReadPostIDs(completion: { (postArrayRead) in
                var readBoolean: Bool = false
                for (key, value) in postDict {
                    for item in postArrayRead {
                        if key == item {
                            readBoolean = true
                        }
                    }
                    postArray.append(Post(id: key, username: value["username"] as! String, postImagePath: value["imagePath"] as! String, thread: value["thread"] as! String, dateString: value["date"] as! String, read: readBoolean))
                    readBoolean = false
                }
